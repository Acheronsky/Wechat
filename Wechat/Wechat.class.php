...
<Nonce><![CDATA[%s]]></Nonce>
	...
	public function batchUpdateGroupMembers($groupid,$openid_list){
         ...
         * @param type $type 消息类型 ，文本为text，图片为image，语音为voice，视频消息为video，音乐消息为music，图文消息（点击跳转到外链）为news，图文消息（点击跳转到图文消息页面）为mpnews，卡券为wxcard，小程序为miniprogrampage
         * @param type $content 发送内容 string  or  array
         * @return boolean
         */
	public function sendCustomMessage($openid, $type, $content){
                $data = $this->sendCustomMessageContent($openid, $type, $content);
		if (!$this->access_token && !$this->checkAuth()) return false;
         ...
         * @param type $content
         */
        public function sendCustomMessageContent($openid, $type, $content){
            $data["touser"] = $openid;
            $data["msgtype"] = $type;
            switch ($type){
                case "text"://文本消息
